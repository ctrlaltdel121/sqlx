...
func loadDefaultFixtureContext(ctx context.Context, db *DB, t *testing.T) {
	...
	tx.MustExecContext(ctx, tx.Rebind("INSERT INTO person (first_name, last_name, email) VALUES (?, ?, ?)"), "Jason", "Moiron", "ctrlaltdel121@ctrlaltdel121.net")
...
func TestUsageContext(t *testing.T) {
	RunWithSchemaContext(context.Background(), defaultSchema, t, func(ctx context.Context, db *DB, t *testing.T) {
		...
		if jason.Email != "ctrlaltdel121@ctrlaltdel121.net" {
			t.Errorf("Expecting Email of ctrlaltdel121@ctrlaltdel121.net, got %s", jason.Email)
		}
